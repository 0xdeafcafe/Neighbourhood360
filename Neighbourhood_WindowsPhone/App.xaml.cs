...
namespace Neighbourhood_WindowsPhone
{
    public partial class App : Application
    {
        ...
        /// <summary>
        /// Provides access to the Xbox Debug Communication Library
        /// </summary>
        public static XBDM XBDM = new XBDM();
        ...
        public App()
        {
            ...
            // Set up navigation connection checks
        ...
        // Code to execute when a navigation is done
        private void RootFrame_Navigating(object sender, NavigatingCancelEventArgs e)
        {
            if (e.Uri.ToString().ToLower().Contains("startup.xaml"))
                return;
            ...
            if (CredentialVault != null &&
                !String.IsNullOrEmpty(CredentialVault.ConsoleIP) &&
                CredentialVault.ConsolePort != 0)
                credsFound = true;

            // If there are none, go to startup page
            if (!credsFound)
            {
                e.Cancel = true;
                RootFrame.Dispatcher.BeginInvoke(delegate
                {
                    RootFrame.Navigate(new Uri("/Pages/StartUp.xaml", UriKind.Relative));
                });
            }

            // Check connection
            XBDM.Disconnect();
            XBDM.ConsoleIP = CredentialVault.ConsoleIP;
            XBDM.ConsolePort = CredentialVault.ConsolePort;

            bool connectionOpen = XBDM.Connect();
            if (!connectionOpen)
            {
                e.Cancel = true;
                RootFrame.Dispatcher.BeginInvoke(delegate
